#include <iostream>
#include <cykohonen.h>
#include <misc.h>

using namespace std;

int main ( int argc, char* argv [] )
{
  CyKohonen *koh = new CyKohonen();
  koh->NInp = 2; //2 - нейрона, 2 - входа
  koh->Nn = 2;
  koh->Init();

//  DBG("Начальная матрица связей");
//  for (int i=0; i<koh->NInp;i++ )
//    DBG("%f ", koh->W[i][0]);

  vector <float> in(koh->NInp);
  vector <float> out(koh->Nn);
  koh->SetOut(&out);
  for (int n = 0; n <10000; n++)
    {
      int num = 0;
      /* Случайно 0 или 1 */
      in[0] = (int)Random(0,2); //RANDOM( 2 );
      if (in[0]==0) in[1] = 1;
        else in[1]=0; 
      koh->SetIn(&in);      
      koh->ModifyWeights();
    }

  koh->SaveNet();
  delete koh;
  koh = new CyKohonen();
  koh->LoadNet();
  koh->SetOut(&out);

//  DBG("Конечная матрица связей");
//  for (int i=0; i<koh->NInp;i++ )
//    DBG("%f ", koh->W[i][0]);

  cout << "               CYKOHONEN_TEST2               " << endl;
  cout << "-------------------------------------------------" << endl;
  cout << "Нейронной сети предъявляется вектор состоящий из" << endl;
  cout << "1 0 или 0 1. Сеть должна научиться разделять на 2" << endl;
  cout << "класса входящие вектора." << endl;
  cout << "-------------------------------------------------" << endl;
  cout << " ВОПРОС                  ОТВЕТ    " << endl;
  cout << "-------------------------------------------------" << endl;

  for (int n = 0; n <25; n++)
    {
      int num = 0;
      in[0] = (int)Random(0,2); //RANDOM( 2 );
      if (in[0]==0) in[1] = 1;
        else in[1]=0; 
      cout << in[0] << " " << in[1];
	  
      koh->SetIn(&in);
      koh->ProcessLayer();
      printf("                     %.4f %.4f\n",out[0], out[1]);
    }
  
  delete koh;
  return 0;
}
